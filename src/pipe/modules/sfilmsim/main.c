...
// Mean dye-layer scatter widths; vkdt blur is scalar.
    ...
    dt_module_t *module)
{
  ...
  module->connector[s_port_input].roi.full_wd /= s;
  module->connector[s_port_input].roi.ht /= s;
  module->connector[s_port_input].roi.full_ht /= s;
}
...
build_curvewarp_node(dt_graph_t *graph, dt_module_t *module, const filmsim_plan_t *plan)
{
  ...
  const int id_curvewarp = dt_node_add(graph, module, "sfilmsim", "dirlut", 1, 1, 1, 0, 0, 2,
...
#define FILMSIM_PREP_BUF_SIZE 2048
  ...
  const int id_setup = dt_node_add(graph, module, "sfilmsim", "setup", 1, 1, 1, 0, 0, 2,
      "sfilmsim",   "read",  "*",    "*",    dt_no_roi,
      "prep",      "write", "ssbo", "f32",  &roi_prep_buf);
  filmsim_copy_module_port(graph, module, s_port_profile, id_setup, 0);
...
{
  dt_roi_t roi_scene = module->connector[s_port_input].roi;
  const int id_expose = dt_node_add(graph, module, "sfilmsim", "expose", plan->iwd, plan->iht, 1, 0, 0, 4,
      "spectrum", "read", "rgba", "f16", dt_no_roi,
      "prep", "read", "*", "*", dt_no_roi,
      "coupler", "write", "rgba", "f16", &roi_scene,
      "exp", "write", "rgba", "f16", &roi_scene);
    ...
    int id_expose, const filmsim_plan_t *plan)
{
  ...
  dt_roi_t roi_scene = module->connector[s_port_input].roi;
  dt_roi_t roi_density = module->connector[s_port_output].roi;
  int id_dev1;
  if(plan->halation)
    id_dev1 = dt_node_add(graph, module, "sfilmsim", "halin", plan->iwd, plan->iht, 1, 0, 0, 5,
        "exp",       "read",  "*",    "*",    dt_no_roi,
        "prep",      "read",  "*",    "*",    dt_no_roi,
        "cwarp",     "read",  "*",    "*",    dt_no_roi,
        "coupler",   "read",  "*",    "*",    dt_no_roi,
        "output",    "write", "rgba", "f16", &roi_scene);
  else
  {
    id_dev1 = dt_node_add(graph, module, "sfilmsim", "develop", plan->owd, plan->oht, 1, 0, 0, 5,
        "exp",       "read",  "*",    "*",    dt_no_roi,
        "prep",      "read",  "*",    "*",    dt_no_roi,
        "cwarp",     "read",  "*",    "*",    dt_no_roi,
        "coupler",   "read",  "*",    "*",    dt_no_roi,
        "output",    "write", "rgba", "f16", &roi_density);
  }
  ...
  if (id_curvewarp >= 0)
    ...
    filmsim_copy_module_port(graph, module, s_port_profile, id_dev1, plan->halation ? 2 : 2);
...
build_scatter_stage(dt_graph_t *graph, dt_module_t *module, int id_dev1, const filmsim_plan_t *plan)
{
  ...
  const int id_scatter = dt_node_add(graph, module, "sfilmsim", "scatter", plan->iwd, plan->iht, 1, 0, 0, 6,
...
{
  dt_roi_t roi_density = module->connector[s_port_output].roi;
  const int id_dev2 = dt_node_add(graph, module, "sfilmsim", "halout", plan->owd, plan->oht, 1, 0, 0, 10,
      "exp",       "read",  "*",    "*",    dt_no_roi,
      "prep",      "read",  "*",    "*",    dt_no_roi,
      "cwarp",     "read",  "*",    "*",    dt_no_roi,
      "output",    "write", "rgba", "f16", &roi_density,
  ...
  if (id_curvewarp >= 0)
    ...
    filmsim_copy_module_port(graph, module, s_port_profile, id_dev2, 2);
...
static void
build_scan_stage(dt_graph_t *graph, dt_module_t *module, int id_setup, int id_density, const filmsim_plan_t *plan)
{
  const int id_scan = dt_node_add(graph, module, "sfilmsim", "scan", plan->owd, plan->oht, 1, 0, 0, 3,
      "density", "read", "rgba", "f16", dt_no_roi,
      "prep", "read", "*", "*", dt_no_roi,
      "output", "write", "rgba", "f16", &module->connector[s_port_output].roi);
  CONN(dt_node_connect_named(graph, id_density, "output", id_scan, "density"));
  CONN(dt_node_connect_named(graph, id_setup, "prep", id_scan, "prep"));
    ...
    dt_module_t *module)
{
  ...
  build_scan_stage(graph, module, id_setup, id_dev2, &plan);
}
